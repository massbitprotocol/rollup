use super::circuit_builder::{
    state_db::{self, AccountTree},
    Block, CircuitBuilder, CircuitsParams, CodeDB, StateDB,
};
...
impl BlockData {
    /// Generate a new CircuitBuilder initialized with the context of the
    /// BlockData.
    pub fn new_circuit_builder(&self) -> CircuitBuilder {
        CircuitBuilder::new(
            self.sdb.clone(),
            self.code_db.clone(),
            Block::new(
