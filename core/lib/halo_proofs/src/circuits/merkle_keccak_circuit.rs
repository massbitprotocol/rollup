...
pub struct MerkleCircuitConfig<F: Field> {
    ...
    s_swap: Selector,
    s_hash: Selector,
    _value: Value<F>, //keccak_circuit: KeccakCircuitConfig<F>,
...
impl<F: Field> MerkleChip<F> {
    fn new(config: MerkleCircuitConfig<F>) -> Self {
        MerkleChip { config }
    }

    fn configure(cs: &mut ConstraintSystem<F>) -> MerkleCircuitConfig<F> {
        ...
        let keccak_table = KeccakTable::construct(cs);
        ...
        MerkleCircuitConfig {
            ...
            s_swap,
            s_hash,
            _value, //keccak_circuit, //perm_digest,
        }
    }
...
impl<F: Field> MerkleCircuit<F> {
    ...
    pub fn construct(leaf: Option<F>, path: Vec<(F, bool)>) -> Self {
        let inputs = Self::create_keccak_witness(&path);
        // let inputs = vec![
        //     //vec![],
        //     //(0u8..1).collect::<Vec<_>>(),
        ...
        //     (0u8..128).collect::<Vec<_>>(),
        // ];
        println!("Keccak inputs: {:?}", &inputs);
        //let keccak_circuit = KeccakCircuit::new(None, inputs);
        Self {
            leaf,
            path,
            //keccak_circuit,
    ...
    fn create_keccak_witness(path: &Vec<(F, bool)>) -> Vec<Vec<u8>> {
        let mut result: Vec<Vec<u8>> = vec![];
        let mut digest: Vec<u8> = vec![];
        path.iter().for_each(|(elem, c_bit)| {
            digest = Self::join_layer_nodes(&digest, elem, c_bit);
            result.push(digest.clone());
        });
        result
    }
    fn join_layer_nodes(digest: &Vec<u8>, path_elem: &F, c_bit: &bool) -> Vec<u8> {
        let mut bytes: Vec<u8> = vec![];
        if *c_bit {
            digest.iter().for_each(|b| bytes.push(*b));
            path_elem
                .to_repr()
                .as_ref()
                .iter()
...
impl<F: Field> SubCircuit<F> for MerkleCircuit<F> {
    ...
    fn synthesize_sub(
        ...
        let challengers = Challenges::mock(Value::known(F::one()), Value::known(F::one()));
        // self.keccak_circuit
