...
use super::circuit_builder::{CircuitBuilder, CircuitsParams};
use super::merkle_circuit::{self, MerkleCircuit, MerkleCircuitConfig, MerkleCircuitConfigArgs};
...
use super::witness::{block_convert, Block};
use crate::keccak_circuit::keccak_packed_multi::{KeccakCircuit, KeccakCircuitConfig};
use crate::table::{
...
pub const KECCAK_CIRCUIT_SIZE: u32 = 0x8;
/// Configuration of the Main Circuit
#[derive(Clone)]
pub struct SimplifiedCircuitConfig<
    F: Field,
    const MAX_TXS: usize,
...
impl<F: Field, const MAX_TXS: usize, const MAX_CALLDATA: usize, const MAX_RWS: usize> Circuit<F>
    for SimplifiedCircuit<F, MAX_TXS, MAX_CALLDATA, MAX_RWS>
{
    type Config = SimplifiedCircuitConfig<F, MAX_TXS, MAX_CALLDATA, MAX_RWS>;
    ...
    fn configure(meta: &mut ConstraintSystem<F>) -> Self::Config {
        ...
        let (keccak_circuit, challenges) = KeccakCircuit::configure(meta);
        //let merkle_circuit = MerkleCircuitConfig::new(meta, MerkleCircuitConfigArgs {});
        // let merkle_circuit = MerkleCircuitConfig::new(
        //     meta,
    ...
    fn synthesize(
            ...
            .synthesize_sub(&config.keccak_circuit, &challenges, &mut layouter)
        {
            Ok(_) => Ok(()),
            Err(err) => {
                println!("MerkleCircuit error {:?}", &err);
                Err(err)
            }
        };
        // match self
        //     .merkle_circuit
        //     .synthesize_sub(&config.merkle_circuit, &challenges, &mut layouter)
        // {
        //     Ok(_) => Ok(()),
        //     Err(err) => {
        //         println!("MerkleCircuit error {:?}", &err);
        ...
        println!("Simplified circuit finished synthesize!!!");
        Ok(())
    }
}

impl<const MAX_TXS: usize, const MAX_CALLDATA: usize, const MAX_RWS: usize>
    SimplifiedCircuit<Fr, MAX_TXS, MAX_CALLDATA, MAX_RWS>
{
    ...
    pub fn build(
        ...
        let ret = Self::build_from_circuit_builder(&builder, account_tree, account_pos)?;
        println!("Circuit built in {:?}", start.elapsed());
        Ok((ret.0, ret.1, ret.2, builder))
    }

    /// From CircuitBuilder, generate a SuperCircuit instance with all of
    ...
    pub fn build_from_circuit_builder(
        ...
        let num_rows_required =
            SimplifiedCircuit::<_, MAX_TXS, MAX_CALLDATA, MAX_RWS>::get_num_rows_required(&block);
        ...
        // let mut merkle_circuit = MerkleCircuit::new_from_block(&block);
        let leaf = account_tree
            .get(account_pos)
            .and_then(|acc| acc.address.to_scalar());
        let merkle_path = account_tree.merkle_path(account_pos);
        println!(
            "Merkle path: {:?} of pos {:?} from tree {:?}",
            &merkle_path, account_pos, account_tree
        );
        let mut merkle_circuit = MerkleCircuit::<Fr>::construct(leaf, merkle_path);
        let inputs = vec![
            //vec![],
            //(0u8..1).collect::<Vec<_>>(),
        ...
        println!("Keccak inputs: {:?}", &inputs);
        let keccak_circuit = KeccakCircuit::new(Some(2usize.pow(KECCAK_CIRCUIT_SIZE)), inputs);
        //let keccak_circuit = KeccakCircuit::new_from_block(&block);

        let circuit = SimplifiedCircuit::<_, MAX_TXS, MAX_CALLDATA, MAX_RWS> {
            evm_circuit,
            state_circuit,
            //merkle_circuit,
            keccak_circuit,
...
#[cfg(test)]
mod simplified_circuit_tests {
    use super::*;
    use crate::circuits::circuit_builder::state_db::{generate_account_tree, Account};
    ...
    fn main_circuit_degree() {
        ...
        SimplifiedCircuit::<_, 1, 32, 256>::configure(&mut cs);
    ...
    #[ignore]
    #[test]
    fn serial_test_simplified_circuit() {
        ...
        let mut account_tree = generate_account_tree(3);
        println!("Account tree items length {:?}", account_tree.items.len());
        account_tree.insert(0, Account::from_address(addr_b));
        println!("Account tree items length {:?}", account_tree.items.len());
        let mut rng = rand::thread_rng();
        let len = account_tree.items.len() as u32;
        let account_pos: u32 = rng.gen_range(0..len); // generates a float between 0 and len - 1
        account_tree.insert(account_pos, Account::from_address(addr_a));
        println!("Account tree items length {:?}", account_tree.items.len());
        ...
        let (k, circuit, instance, _) =
            SimplifiedCircuit::<_, 1, 32, 256>::build(account_tree, account_pos, block).unwrap();
        ...
        match MockProver::run(18, &circuit, instance.clone()) {
            Ok(prover) => {
                println!("MockProver run ok");
                let res = prover.verify_par();
                println!("Finish prover verified par {:?}", &res);
                if let Err(err) = res {
        ...
        println!("Finished MockProver run");
