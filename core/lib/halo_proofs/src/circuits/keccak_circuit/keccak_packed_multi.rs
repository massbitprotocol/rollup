...
impl<F: Field> KeccakCircuit<F> {
    ...
    pub(crate) fn generate_witness(&self, challenges: Challenges<Value<F>>) -> Vec<KeccakRow<F>> {
        let keccak_rows = multi_keccak(self.inputs.as_slice(), challenges, self.capacity())
            .expect("Too many inputs for given capacity");
        keccak_rows
    }
    pub fn get_hashed_data(&self) -> Vec<Value<F>> {
        let mut outputs = vec![];
        let challenges = Challenges::mock(Value::known(F::one()), Value::known(F::one()));
        let keccak_rows = multi_keccak(self.inputs.as_slice(), challenges, self.capacity())
            .expect("Too many inputs for given capacity");
        let ouputs = keccak_rows
            .iter()
            .map(|row| row.hash_rlc.clone())
            .collect::<Vec<Value<F>>>();
        outputs
    }
}
...
mod tests {
    ...
    fn verify<F: Field>(k: u32, inputs: Vec<Vec<u8>>, success: bool) {
        ...
        let hashed_data = circuit.get_hashed_data();
    ...
    fn packed_multi_keccak_simple() {
        ...
        let inputs = vec![
            ...
            (0u8..64).collect::<Vec<_>>(),
            (0u8..64).collect::<Vec<_>>(),
            (65u8..128).collect::<Vec<_>>(),
            (129u8..192).collect::<Vec<_>>(),
            (0u8..64).collect::<Vec<_>>(),
        ];
