...
use super::state_db::{CodeDB, StateDB};
use super::{
    get_call_memory_offset_length, get_create_init_code, Block, BlockContext, CodeSource, ExecStep,
    Transaction, TransactionContext,
};
use bus_mapping::circuit_input_builder::{
    Call, CallContext, CallKind, CopyEvent, ExecState, ExpEvent,
};
...
impl<'a> CircuitStateRef<'a> {
    ...
    pub fn push_op_reversible<T: Op>(
        ...
        //Todo: check reversible operator
    ...
    pub(crate) fn get_step_err(
        ...
        if let Some(error) = &step.error {
            return Ok(Some(Block::get_step_reported_error(&step.op, error)));
        }
