...
mod state_ref;
mod transaction;
//use crate::witness::ExecStep;
use super::opcodes::{gen_associated_ops, gen_begin_tx_ops, gen_end_tx_ops};
use access::{gen_state_access_trace, Access, AccessSet, AccessValue};
pub use block::{Block, BlockContext};
use bus_mapping::circuit_input_builder::{CallContext, CodeSource, ExecStep};
use bus_mapping::error::Error;
use bus_mapping::operation::{CallContextField, Operation, RWCounter, StartOp, RW};
use bus_mapping::rpc::GethClient;
use eth_types::sign_types::SignData;
use eth_types::sign_types::{pk_bytes_le, pk_bytes_swap_endianness};
use eth_types::H256;
use eth_types::{self, geth_types, Address, GethExecStep, GethExecTrace, ToWord, Word};
use ethers_providers::JsonRpcClient;
use log::warn;
pub use state_db::{Account, CodeDB, StateDB};
pub use state_ref::CircuitStateRef;
...
/// the only entry point to it. The `CircuitBuilder` works in several
/// steps:
///
...
impl<'a> CircuitBuilder {
    /// Create a new CircuitBuilder from the given `eth_block` and
    /// `constants`.
    ...
    /// Obtain a mutable reference to the state that the `CircuitBuilder`
    ...
    pub fn handle_block(
        ...
        self.block
            .txs()
            .iter()
            .for_each(|tx| println!("Handled Transaction steps {:?}", tx.steps()));
    ...
    fn handle_tx(
        ...
        println!(
            "Handle transaction {:?} with traces {:?}",
            eth_tx, geth_trace
        );
        for (index, geth_step) in geth_trace.struct_logs.iter().enumerate() {
            let mut state_ref = self.state_ref(&mut tx, &mut tx_ctx);
            //println!("Handle {}th geth_step {:?}", index, &geth_step);
...
/// the necessary information and using the CircuitBuilder.
...
impl<P: JsonRpcClient> BuilderClient<P> {
    ...
    pub fn gen_inputs_from_state(
        ...
        let mut builder = CircuitBuilder::new(sdb, code_db, block);
    ...
    pub async fn gen_inputs(
        &self,
        block_num: u64,
    ) -> Result<(CircuitBuilder, eth_types::Block<eth_types::Transaction>), Error> {
