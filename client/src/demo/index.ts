...
async function logL1Balances() {
  //Layer 1 balances
  let aliceBalances = [], bobBalances = [];
...
async function initERC20Data() {  
  ...
  for (const token of erc20Tokens) {
    ...
    console.log(`L2#Transfer 250 ERC20 token ${token} from Alice with address ${aliceAddress} to Bob with address ${bobAddress}`);
