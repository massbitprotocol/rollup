...
export class L2Client {
  ...
  async transferERC20Token(sender: ethers.Wallet, receipent: ethers.Wallet, token: TokenLike, amount: BigNumber,) {
    ...
    if (l2SenderWallet.address() === l2ReceipentWallet.address()) {
        ...
        expect(senderBefore.sub(senderAfter).eq(amount.add(fee)), 'Transfer failed (incorrect sender balance)').to.be.true;
  ...
  async withdrawToEthereum(etherWallet: ethers.Wallet, token: TokenLike, amount: BigNumber, fastProcessing?: boolean) {
